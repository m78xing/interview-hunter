from nowcoder_spider import nowcoderSpider
    ...
    def run(self, keywords, max_per_keyword, days_range):
        ...
        try:
            ...
            for raw in raw_interviews:
                if not raw.content:
                    raw.content = self.spider.get_detail(raw.url) or ""
                
                if not raw.content:
                    raw.content = f"标题: {raw.title}\n来源: {raw.url}"
                processed = self.process(raw.title, raw.content, raw.url, "牛客网", raw.date)
                item = InterviewItem(
                    id="",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="牛客网爬虫工具")
